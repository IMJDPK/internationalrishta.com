...
| Bureau directory UI | `src/app/[locale]/bureau/page.tsx` — live Supabase directory with URL search (`?q=`, `?city=`) |
| Registration UI | `src/app/[locale]/bureau/register/page.tsx` — **POST `/api/bureau/register`** via `BureauRegisterForm` |
| Admin bureau tab | `src/app/admin/dashboard/page.tsx` — **POST `/api/admin/approve-bureau`** with commission fields |
| Middleware | `src/middleware.ts` — next-intl + `?ref=` → `ir_bureau_ref` cookie |
| Auth callback | `src/app/auth/callback/route.ts` — OAuth + `attributeReferralForUser` |
...
`is_approved=false`, generates unique `referral_code`; `payment_receipt_path` optional in MVP (see `contracts/bureau-register-api.md`).
